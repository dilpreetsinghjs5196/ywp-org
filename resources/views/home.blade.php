...
@section('content')
            ...
            <div class="swiper-wrapper">
                @php
                    ...
                    if (isset($contents['hero'])) {
                        foreach ($contents['hero'] as $key => $value) {
                            if (preg_match('/slide(\d+)_(\w+)/', $key, $matches)) {
                                $index = $matches[1];
                                $field = $matches[2];
                                ...
                                <div class="col-xl-7">
                                    <div class="main-slider__content">
                                        ...
                                        <div class="main-slider-shape-1"><img
                                                src="{{ asset('assets/images/shapes/main-slider-shape-1.png') }}"
                                                class="float-bob-x" alt=""></div>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <section class="we-believe">
        <div class="we-believe-map" style="background-image: url({{ asset('assets/images/shapes/we-believe-map.png') }});">
        </div>
        <div class="container">
            <div class="section-title text-center">
                ...
                <h2 class="section-title__title">
                    {!! $contents['we_believe']['title'] ?? 'We believe that we can save <br> more lives with you' !!}
                </h2>
            </div>
            <div class="row">
                <div class="col-xl-12">
                    @php
                        $believeIcons = [];
                        if (isset($contents['we_believe'])) {
                            foreach ($contents['we_believe'] as $key => $value) {
                                if (preg_match('/icon(\d+)_(\w+)/', $key, $matches)) {
                                    $believeIcons[$matches[1]][$matches[2]] = $value;
                                }
                            }
                            ksort($believeIcons);
                        }
                    ...
                    <ul class="list-unstyled we-believe__list">
                        @foreach($believeIcons as $index => $icon)
                            @php
                                $delay = ($index * 100) . 'ms';
                                $link = $icon['link'] ?? '#';
                                // Check if it's a relative link or absolute
                                if (!preg_match("~^(?:f|ht)tps?://~i", $link) && !str_starts_with($link, '#')) {
                                    $link = url($link);
                                }
                            @endphp
                            <a href="{{ $link }}">
                                <li class="we-believe__single wow fadeInUp" data-wow-delay="{{ $delay }}">
                                    <div class="we-believe__icon">
                                        <span class="{{ $icon['class'] ?? 'icon-heart' }}"></span>
                                    </div>
                                    <h3 class="we-believe__title"><a href="{{ $link }}">{{ $icon['title'] ?? '' }}</a></h3>
                                </li>
                            </a>
                        @endforeach
                    </ul>
        ...
        <div class="container">
            <div class="row">
                <div class="col-xl-6">
                    @php
                        $welcomeOne = [];
                        if (isset($contents['welcome_one'])) {
                            foreach ($contents['welcome_one'] as $key => $value) {
                                $welcomeOne[$key] = $value;
                            }
                        }
                    @endphp
                    <div class="welcome-one__left">
                        <div class="welcome-one__img-box wow slideInLeft" data-wow-delay="100ms" data-wow-duration="2500ms">
                            <div class="welcome-one__img">
                                <img src="{{ asset($welcomeOne['image1'] ?? 'images/welcome-one-img.jpg') }}" alt="">
                            </div>
                            <div class="welcome-one__small-img">
                                <img src="{{ asset($welcomeOne['image2'] ?? 'images/welcome-one-small-img.jpg') }}" alt="">
                            </div>
                ...
                <div class="col-xl-6">
                    <div class="welcome-one__right">
                        <div class="section-title text-left">
                            ...
                            <h2 class="section-title__title">
                                {!! $welcomeOne['title'] ?? 'YWP; is creating a safe space for the youth.' !!}
                            </h2>
                        </div>

                        <p class="welcome-one__text-2">
                            {{ $welcomeOne['description'] ?? 'We at YWP; believe in providing a safe space to individuals struggling with mental health issues...' }}
                        </p>
                        <div class="welcome-one__btn-box">
                            <a href="{{ url($welcomeOne['discover_link'] ?? 'about') }}"
                                class="thm-btn welcome-one__btn">Discover More</a>
                            <div class="welcome-one__video-link">
                                @php
                                    $videoLink = $welcomeOne['video_link'] ?? 'https://www.youtube.com/c/YoureWonderfulProject';
                                    $isChannel = str_contains($videoLink, 'youtube.com/c/') || str_contains($videoLink, 'channel/');
                                @endphp
                                <a href="{{ $videoLink }}" class="{{ $isChannel ? '' : 'video-popup' }}" @if($isChannel)
                                target="_blank" @endif>
    ...
    <style>
        ...
        .causes-one .causes-one__text {
            flex-grow: 1;
            margin-bottom: 0 !important;
            min-height: 100px;
        }

        .causes-one .causes-one__title {
            min-height: 60px;
            display: flex;
            align-items: center;
        }
                        ...
                        @foreach($campaigns as $camp)
                            <div class="causes-one__single">
                                <div class="causes-one__img">
                                    <img src="{{ asset($camp->image ?? 'images/campaigns-1-1.jpg') }}" alt="">
        ...
        <div class="container">
            <div class="row">
                <div class="col-xl-6 col-lg-6">
                    <!--Two Boxes Single-->
                    <div class="two-boxes__single wow slideInLeft" data-wow-delay="100ms" data-wow-duration="2500ms">
                        ...
                        <div class="two-boxes__content" style="padding-bottom:5px;">
                            <a href="mailto:peersupport@yourewonderfulproject.org">
                                <h3 class="two-boxes__title">Free Therapy</h3>
                            </a>
                            <br />
            ...
            <div class="row">
                <div class="col-xl-12">
                    <div class="fundraishing__inner">
                        ...
                        <h2 class="fundraishing__title">Fundraising for <span>mental health awareness</span> and <br>
                            accessability</h2>
                        <div class="fundraishing__btn-box">
                            <a href="https://pledge.yourewonderfulproject.org/" class="thm-btn fundraishing__btn"
                                target="_blank">Start Donating now</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <section class="featured-campaigns">
        <div class="container">
            ...
            <div class="row">
                <center>
                    <h2 class="section-title__title">Research Papers</h2>
                </center><br><br>
                <div class="col-xl-12 col-xxl-12">
                    <div class="featured-campaigns__single">
                        <div class="featured-campaigns__content">
                            <h3 class="featured-campaigns__title">Mental Health Awareness amongst School Students across
                                Delhi, NCR: A Pilot Study</h3>
                            <p style="line-height:16px;">The study was aimed at initiating a conversation about the
                                importance of mental health among school children. This project has been founded with the
                                aim to equip 6th-8th graders with the knowledge of common mental health issues through the
                                process of a) creating awareness, b) encouraging acceptance, c) ensuring prevention, and d)
                                assisting with cure. For this study, a qualitative design was used. This study is a pilot
                                study that will further help to understand how to ensure the effectiveness of the SPEAK
                                program, as well as conduct more enhanced workshops around mental health awareness among
                                school students.</p>
                            <div class="events__single">
                                <h3 class="events__title"><a
                                        href="https://drive.google.com/file/d/13W7tESwu72J07Be16zSR2BkqLT-Nt_mW/view?usp=sharing"
        ...
        <div class="container">
            <div class="row">
                ...
                @php
                    ...
                    $faqCount = count($faqs);
                    $leftFaqs = array_slice($faqs, 0, ceil($faqCount / 2), true);
                    $rightFaqs = array_slice($faqs, ceil($faqCount / 2), null, true);
                @endphp
                    ...
                    <div class="faq-page__single">
                        <div class="accrodion-grp faq-one-accrodion" data-grp-name="faq-one-accrodion-1">
                            @foreach($leftFaqs as $index => $faq)
                                <div class="accrodion {{ $loop->last ? 'last-chiled' : '' }}">
                    ...
                    <div class="faq-page__single">
                        <div class="accrodion-grp faq-one-accrodion" data-grp-name="faq-one-accrodion-2">
                            @foreach($rightFaqs as $index => $faq)
                                <div class="accrodion {{ $loop->last ? 'last-chiled' : '' }}">
                                    <div class="accrodion-title">
                                        <h4>{{ count($leftFaqs) + $loop->iteration }}. {{ $faq['question'] ?? '' }}</h4>
                                    </div>
                                    <div class="accrodion-content">
                                        <div class="inner">
                                            <p>{{ $faq['answer'] ?? '' }}</p>
